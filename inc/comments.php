...
function get_comment_parent_info($comment){
	if (!lyrargon_comment_options('show_comment_parent_info')){
		return "";
	}
...
//评论展示相关选项的访问器（封装原 $GLOBALS['lyrargon_comment_options']，避免全局状态）
function lyrargon_comment_options($key = null) {
	static $options = null;
	if ( $options === null ) {
		$options = array(
			'enable_upvote'                      => ( get_option( 'lyrargon_enable_comment_upvote', 'false' ) == 'true' ),
			'enable_pinning'                     => ( get_option( 'lyrargon_enable_comment_pinning', 'false' ) == 'true' ),
			'current_user_can_moderate_comments' => current_user_can( 'moderate_comments' ),
			'show_comment_parent_info'           => ( get_option( 'lyrargon_show_comment_parent_info', 'true' ) == 'true' ),
		);
	}
	if ( $key === null ) {
		return $options;
...
function argon_comment_format($comment, $args, $depth){
	...
	<li class="comment-item" id="comment-<?php comment_ID(); ?>" data-id="<?php comment_ID(); ?>" data-use-markdown="<?php echo get_comment_meta(get_comment_ID(), "use_markdown", true);?>">
		<div class="comment-item-left-wrapper">
			...
			<?php if (lyrargon_comment_options('enable_upvote')){ ?>
		...
		<div class="comment-item-inner" id="comment-inner-<?php comment_ID();?>">
			<div class="comment-item-title">
				<div class="comment-name">
					...
					<?php if (lyrargon_comment_options('enable_pinning') && get_comment_meta(get_comment_ID(), "pinned", true) == "true"){
			...
			<div class="comment-operations">
				<?php if (lyrargon_comment_options('enable_pinning') && lyrargon_comment_options('current_user_can_moderate_comments') && is_comment_pinable(get_comment_ID())) {
