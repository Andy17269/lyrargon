				...
				return (
					<div>
						...
						{getUndeletedItemsCount(props.attributes.list) > 1 &&
							<Button className="is-tertiary tab-remove-item-btn" onClick={() => removeNode(node.id)} style={{color: "var(--themecolor)"}}><span className="dashicon dashicons dashicons-trash"></span>&nbsp;{__("移除此项")}</Button>
						}
					</div>
				)
			}
				...
				<div className="argon-tabpanel">
					<div className="tabpanel-header nav-wrapper">
						<ul className="nav nav-pills nav-fill" role="tablist">
							{tabTitleNodeList}
							<li className="nav-item">
								<a className="nav-link" onClick={addNode} style={{color: "#fff", backgroundColor: "var(--themecolor)", cursor: "pointer"}}>{__("+ 添加 Tab")}</a>
							</li>
						</ul>
					</div>
