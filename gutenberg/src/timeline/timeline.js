				...
				<div class="argon-timeline-node" key={node.id.toString()}>
					...
					<div class="argon-timeline-card bg-gradient-secondary shadow-sm">
						...
						{getUndeletedItemsCount(props.attributes.list) > 1 &&
							<Button className="is-tertiary timeline-remove-item-btn" onClick={() => removeNode(node.id)} style={{color: "var(--themecolor)"}}><span class="dashicon dashicons dashicons-trash"></span></Button>
						}
					</div>
				</div>
			);
		...
		return (
			<div>
				<div class="argon-timeline">
					{nodelist}
					<Button className="is-primary" onClick={addNode}  style={{marginTop: 8, backgroundColor: "var(--themecolor)"}}>{__("+ 添加节点")}</Button>
				</div>
			</div>
		);
	},
