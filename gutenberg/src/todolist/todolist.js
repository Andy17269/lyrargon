...
registerBlockType('argon/todolist', {
	...
	edit: (props) => {
		...
		const nodelist = props.attributes.list.map((node) => {
			...
			return (
				<div class="shortcode-todo custom-control custom-checkbox" key={node.id.toString()}>
					...
					/>
					<Button className="is-tertiary todolist-remove-item-btn" onClick={() => removeNode(node.id)} style={{color: "var(--themecolor)"}}><span class="dashicon dashicons dashicons-trash"></span></Button>
				</div>
			);
		});
		...
		return (
			<div>
				{nodelist}
				<Button className="is-primary" onClick={addNode} style={{marginTop: 8, backgroundColor: "var(--themecolor)"}}>{__("+ 添加条目")}</Button>
			</div>
		);
	},
	save: (props) => {
