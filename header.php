...
<?php
	...
	if (isset($_COOKIE["lyrargon_custom_theme_color"])){
		if (checkHEX($_COOKIE["lyrargon_custom_theme_color"]) && get_option('lyrargon_show_customize_theme_color_picker') != 'false'){
...
?>
<?php
	$cardradius = get_option('lyrargon_card_radius');
	if ($cardradius == ""){
		$cardradius = "4";
	}
...
<head>
	...
	<meta name="argon-enable-custom-theme-color" content="<?php echo (get_option('lyrargon_show_customize_theme_color_picker') != 'false' ? 'true' : 'false'); ?>">
...
<style id="theme_large_radius_css">
	.btn, .form-control, .custom-select, .post-tag, .tag-cloud-link, .badge, .nav-pills .nav-link, .iziToast, .related-post-card {
...
<?php } ?>
<?php 
$card_blur = get_option('lyrargon_card_blur');
if ($card_blur != 'none' && $card_blur != '') {
	$blur_radius = '0px';
	$bg_opacity = '0.85';
	if ($card_blur == 'weak') {
		$blur_radius = '5px';
	} else if ($card_blur == 'medium') {
		$blur_radius = '15px';
	} else if ($card_blur == 'strong') {
		$blur_radius = '30px';
		$bg_opacity = '0.7';
	}
?>
<style id="theme_card_blur_css">
	.card {
	...
	.card::before {
		...
		backdrop-filter: blur(<?php echo $blur_radius; ?>);
		-webkit-backdrop-filter: blur(<?php echo $blur_radius; ?>);
	}
</style>
<?php } ?>
...
<section id="banner" class="banner section section-lg section-shaped">
	...
	<?php if (get_option('lyrargon_banner_background_url') != '') { ?>
		<style>
			section.banner{
...
<div id="float_action_buttons" class="float-action-buttons fabtns-unloaded">
	<button id="fabtn_toggle_sides" class="btn btn-icon btn-neutral fabtn shadow-sm" type="button" aria-hidden="true" tooltip-move-to-left="<?php _e('移至左侧', 'lyrargon'); ?>" tooltip-move-to-right="<?php _e('移至右侧', 'lyrargon'); ?>">
