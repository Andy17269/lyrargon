	...
	<div>
		...
		<script>
			jQuery(document).ready(function($) {
				...
				var $search = $('<input type="text" class="lyrargon-search-box" placeholder="<?php _e('搜索设置...', 'argon'); ?>">');
				var $ul = $('<ul></ul>');
				...
				$sidebar.find('li').click(function() {
					...
					$zt.find('.zoomify-toggle-label').text('<?php _e('展开', 'argon');?>');
					$zt.css('opacity','0.5');
			...
			<table class="form-table">
				<tbody>
					...
					<tr>
						...
						<td>
							<input type="color" class="regular-text" name="argon_theme_color" value="<?php echo get_option('argon_theme_color') == "" ? "#2196f3" : get_option('argon_theme_color'); ?>" style="height:40px;width: 80px;cursor: pointer;"/>
							<input type="text" readonly name="argon_theme_color_hex_preview" value="<?php echo get_option('argon_theme_color') == "" ? "#2196f3" : get_option('argon_theme_color'); ?>" style="height: 40px;width: 80px;vertical-align: bottom;background: #fff;cursor: pointer;" onclick="$('input[name=\'argon_theme_color\']').click()"/>
							<div class="description" style="margin-top: 15px;"><?php _e("选择预置颜色 或", 'argon');?> <span onclick="$('input[name=\'argon_theme_color\']').click()" style="text-decoration: underline;cursor: pointer;"><?php _e("自定义色值", 'argon');?></span>
								<br/><br/><?php _e("预置颜色：", 'argon');?></div>
								<div class="themecolor-preview-container">
									<div class="themecolor-preview-box"><div class="themecolor-preview" style="background:#2196f3;" color="#2196f3"></div><div class="themecolor-name">Lyrargon (<?php _e("默认", 'argon');?>)</div></div>
					...
					<tr>
						...
						<td>
							...
							<p class="description"><?php _e('Lyrargon 主题会根据这里的选项来决定是否默认使用夜间模式。', 'argon');?><br/><?php _e('用户也可以手动切换夜间模式，用户的设置将保留到标签页关闭为止。', 'argon');?></p>
						</td>
					</tr>
					<tr>
					...
					<tr>
						<th><label><?php _e('组件圆角', 'argon');?></label></th>
						<td>
							<label>
								<?php $argon_enable_large_radius = get_option('argon_enable_large_radius');?>
								<input type="checkbox" name="argon_enable_large_radius" value="true" <?php if ($argon_enable_large_radius=='true'){echo 'checked';}?>/>	<?php _e('开启圆角组件', 'argon');?>
							</label>
							<p class="description"><?php _e('开启后将把搜索框、按钮等组件变为胶囊形，将小部件变为圆形。', 'argon');?></p>
						</td>
					</tr>
					<tr>
						<th><label><?php _e('卡片毛玻璃效果 (Lite)', 'argon');?></label></th>
						<td>
							<select name="argon_card_blur">
								<?php $argon_card_blur = get_option('argon_card_blur'); ?>
								<option value="none" <?php if ($argon_card_blur=='none' || $argon_card_blur==''){echo 'selected';} ?>><?php _e('关闭', 'argon');?></option>
								<option value="weak" <?php if ($argon_card_blur=='weak'){echo 'selected';} ?>><?php _e('弱', 'argon');?></option>
								<option value="medium" <?php if ($argon_card_blur=='medium'){echo 'selected';} ?>><?php _e('中', 'argon');?></option>
								<option value="strong" <?php if ($argon_card_blur=='strong'){echo 'selected';} ?>><?php _e('强', 'argon');?></option>
							</select>
							<p class="description"><?php _e('为卡片添加半透明的毛玻璃模糊效果，开启后有微小的性能损耗。', 'argon');?></p>
						</td>
					</tr>
					<tr>
						...
						<td>
							...
							<p class="description"><?php _e('卡片的圆角大小，默认为', 'argon');?> <code>22px</code><?php _e('。建议设置为', 'argon');?> <code>15px</code> - <code>25px</code></p>
						</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td>
							...
							<p class="description"><?php _e('设置针对搜索引擎的 Description Meta 标签内容。', 'argon');?><br/><?php _e('在文章中，Lyrargon 会自动根据文章内容生成描述。在其他页面中，Lyrargon 将使用这里设置的内容。如不填，Lyrargon 将不会在其他页面输出 Description Meta 标签。', 'argon');?></p>
						</td>
					</tr>
					<tr>
					...
					<tr><th class="subtitle"><h3><?php _e('外观', 'argon');?></h3></h3></th></tr>
					<tr>
					...
					<tr style="opacity: 0.5;">
						<th><label><?php _e('如果您想使用其他代码高亮插件，而非 Lyrargon 自带高亮，请前往 "杂项" 打开 "禁用 Lyrargon 代码块样式" 来防止样式冲突', 'argon');?></label></th>
						<td>
						</td>
					</tr>
					...
					<tr style="opacity: 0.5; cursor: pointer;" onclick="var $t=$(this);var $a=$t.find('.zoomify-toggle-arrow');var expanded=$a.text()==='▲';if(expanded){$a.text('▼');$t.find('.zoomify-toggle-label').text('<?php _e('展开', 'argon');?>');$t.css('opacity','0.5');}else{$a.text('▲');$t.find('.zoomify-toggle-label').text('<?php _e('收起', 'argon');?>');$t.css('opacity','1');}$('.zoomify-old-settings').fadeToggle(500);">
						<th><label><span class="zoomify-toggle-label"><?php _e('展开', 'argon');?></span><?php _e('旧版图片放大浏览 (Zoomify) 设置', 'argon');?> <span class="zoomify-toggle-arrow">▼</span></label></th>
						<td>
						</td>
					</tr>
					<style>
					...
					<tr class="zoomify-old-settings" style="display: none;">
						...
						<td>
							...
							<p class="description"><?php _e('Zoomify 和 Fancybox 不能同时开启。', 'argon');?></p><p>在 Lyrargon 3.X.X 版本更新后，旧版图片放大浏览功能将被彻底移除。</p>
						</td>
					</tr>
					<tr class="zoomify-old-settings" style="display: none;">
					...
					<tr>
						...
						<td>
							<p class="description"><strong style="color:#ff0000;"><?php _e('Lyrargon 使用 pjax 方式加载页面 (无刷新加载) , 所以除非页面手动刷新，否则您的脚本只会被执行一次。', 'argon');?><br/>
...
window.pjaxLoaded = function(){
	...
	//do something...
					...
					<tr>
						...
						<td>
							...
							<p class="description"><?php _e('使用 CDN 来加速 Gravatar 在某些地区的访问，填写 CDN 地址，留空则不使用。', 'argon');?><br/>下载加速插件 <a href="https://lyargon.wenlei.top/docs/wp-china-yes" target="_blank">WP-China-Yes</a>，可改善中国大陆地区的资源加载体验。</p>
						</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td>
							...
							<p class="description"><?php _e('使用 Lyrargon Design 风格的登录界面', 'argon');?></p>
						</td>
					</tr>
					<tr>
						...
						<td>
							<p class="description">
								<?php _e('使用 Lyrargon Design 风格的后台界面', 'argon');?><br>
								<?php echo sprintf(__('前往<a href="%s" target="_blank">个人资料</a>页面将 "管理界面配色方案" 设为 "Lyrargon" 即可开启。', 'argon'), admin_url('profile.php'));?>
							</p>
						</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td>
							...
							<p class="description"><?php _e('Google 字体在中国大陆访问可能会阻塞，禁用可以解决页面加载被阻塞的问题。禁用后，Serif 字体将失效。', 'argon');?></p>下载加速插件 <a href="https://lyargon.wenlei.top/docs/wp-china-yes" target="_blank">WP-China-Yes</a>，可改善中国大陆地区的资源加载体验。</p>
						</td>
					</tr>
					<tr>
						<th><label><?php _e('禁用 Lyrargon 代码块样式', 'argon');?></label></th>
						<td>
							...
							<p class="description"><?php _e('如果您启用了其他代码高亮插件，发现代码块样式被 Lyrargon 覆盖，出现了显示错误，请将此选项设为禁用', 'argon');?></p>
						</td>
					</tr>
					<tr>
			...
			<p class="submit">
				...
				<a class="button button-secondary" onclick="importSettings()"><?php _e('导入设置', 'argon');?></a>
				<a class="button button-secondary" onclick="exportSettings()"><?php _e('导出设置', 'argon');?></a>
				</span>
			</p>
		</form>
	</div>
	...
	<script type="text/javascript">
		...
		$('#main_form').on('submit', function(e){
			...
			setThemeoptionsSaveStatus('saving', '<?php _e('正在保存', 'argon');?>');
			$button.prop('disabled', true).val('<?php _e('保存中...', 'argon');?>');
			$.ajax({
				...
				if (response && response.success) {
					setThemeoptionsSaveStatus('success', response.data && response.data.message ? response.data.message : '<?php _e('已保存', 'argon');?>');
					window.setTimeout(function(){
						$('#themeoptions-save-status').removeClass('is-success').addClass('is-visible');
					}, 900);
				} else {
					var message = response && response.data && response.data.message ? response.data.message : '<?php _e('保存失败', 'argon');?>';
					setThemeoptionsSaveStatus('error', message);
				}
			}).fail(function(){
				setThemeoptionsSaveStatus('error', '<?php _e('保存失败，请重试', 'argon');?>');
			}).always(function(){
...
function argon_update_themeoptions(){
	...
	if ($_POST['update_themeoptions'] == 'true'){
		if (!isset($_POST['argon_update_themeoptions_nonce'])){
			if (isset($_POST['argon_themeoptions_ajax'])) {
				wp_send_json_error(array('message' => __('校验失败，请刷新后重试', 'argon')));
			}
			return;
		}
		...
		argon_update_option_checkbox('argon_enable_large_radius');
		...
		if (isset($_POST['argon_themeoptions_ajax'])) {
			wp_send_json_success(array('message' => __('已保存', 'argon')));
		}
	}
}
